...
export function AdminProfileDropdown() {
  ...
  const handleSaveAvatar = async (e: React.FormEvent) => {
    ...
    try {
      ...
      updateUser(updated);
      setAvatarSuccess("Foto avatar admin berhasil diperbarui.");
      setTimeout(() => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className="flex items-center gap-2 p-1 sm:p-1.5 sm:pr-2.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 transition-all text-left shadow-2xs cursor-pointer focus:outline-none focus:ring-2 focus:ring-purple-700/20"
        title="Menu Profil & Akun Superadmin"
      >
        <div className="relative">
          {user?.avatar ? (
            <img
              src={user.avatar}
              alt={user.name}
              className="h-8 w-8 rounded-lg object-cover border border-purple-200 shadow-xs"
            />
          ) : (
            <div className="h-8 w-8 rounded-lg bg-gradient-to-tr from-purple-700 to-indigo-600 text-white font-bold text-xs flex items-center justify-center shadow-xs">
        ...
        <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-white border border-slate-200 shadow-xl py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
          ...
          <div className="px-4 py-2.5 border-b border-slate-100 flex items-center gap-3">
            {user?.avatar ? (
              <img
                src={user.avatar}
                alt={user.name}
                className="h-9 w-9 rounded-xl object-cover border border-purple-200 shrink-0"
              />
            ) : (
              <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-purple-700 to-indigo-600 text-white font-bold text-xs flex items-center justify-center shrink-0">
                {user?.name?.slice(0, 2).toUpperCase() || "SA"}
              </div>
            )}
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-1">
                ...
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-purple-100 text-purple-800 border border-purple-200">
                  Admin
                </span>
              </div>
          ...
          <div className="py-1">
            <button
              ...
              className="w-full flex items-center gap-2.5 px-4 py-2 text-xs font-medium text-slate-700 hover:bg-purple-50 hover:text-purple-900 transition-colors text-left cursor-pointer"
            >
              <UserIcon className="h-4 w-4 text-purple-600" />
              <span>Edit Profil Superadmin</span>
            </button>
      ...
      <Dialog
        ...
        title="Edit Profil Superadmin"
        description="Perbarui informasi nama dan alamat email akun administrator sistem Anda."
      >
      ...
      <Dialog
        ...
        title="Pilih atau Ubah Avatar Admin"
        description="Pilih dari koleksi avatar atau gunakan tautan gambar kustom Anda."
      >
          ...
          <div className="flex items-center gap-4 p-3 bg-slate-50 border border-slate-200 rounded-xl">
            <div className="relative">
              {activeAvatarPreview ? (
                ...
                <div className="h-16 w-16 rounded-2xl bg-gradient-to-tr from-purple-700 to-indigo-600 text-white font-bold text-lg flex items-center justify-center shadow-md">
                  {user?.name?.slice(0, 2).toUpperCase() || "SA"}
                </div>
              )}
              <span className="absolute -top-1 -right-1 bg-purple-700 text-white p-1 rounded-full shadow-xs">
                <Sparkles className="h-3 w-3" />
              </span>
            </div>
            <div>
              <div className="text-xs font-bold text-slate-800">Pratinjau Avatar Admin</div>
              <div className="text-[11px] text-slate-500">
                Avatar ini akan tampil di seluruh portal admin dan navigasi sistem.
              </div>
            </div>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`p-1 rounded-xl border transition-all hover:scale-105 cursor-pointer ${
                      isSelected
                        ? "border-purple-700 ring-2 ring-purple-700/30 bg-purple-50"
      ...
      <Dialog
        ...
        description="Pastikan menggunakan kombinasi kata sandi yang kuat dan aman."
      >
